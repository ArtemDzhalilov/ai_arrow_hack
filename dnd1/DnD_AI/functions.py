...
from .functions_AI import image_generator_DallE, image_generator_StabDiff, ask_world_info_gemini, continue_history_gemini, generate_literature_answer
...
def attack(attacker:Character|Monster, target:Character|Monster, attacker_dice:int = roll_dice(), attacker_is:str = 'player') -> dict:
    ...
    History.objects.create(campaign=attacker.campaign, author='SYSTEM', text=generate_literature_answer(f'D{attacker_dice}. {attacker.name} did {damage_dealt} points of DMG to {target.name}.{was_critical_hit_str}'))
...
def combat_turn(player:Character, monster:Monster, player_dice:int = roll_dice(), monster_dice:int = roll_dice()) -> dict[dict, dict]:
    ...
    History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer(f'{first_by}, {player.name if first=="player" else monster.name} attacks first.'))
...
def player_died_in_combat(player:Character, monster:Monster):
    ...
    History.objects.create(campaign=player.campaign ,author='SYSTEM', text=generate_literature_answer(f'{player.name} died in combat.').save())
...
def player_won_combat(player:Character, monster:Monster):
    ...
    History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer(f'{player.name} killed {monster.name}, got {monster.exp_drop} EXP and {loot}')).save()

    if monster.is_key:
        achievement = f"KEY BOSS {monster.name} WAS SLAIN BY {player.name}."
        player.campaign.objective_completed(achievement)
        History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer(f'[ACHIEVEMENT - {achievement}]')).save()
...
def command_executer(prompt:str|list, player:Character, target:Monster) -> tuple[bool, dict]:
    ...
    if len(prompt) == 0 or prompt == '' or prompt == '\n':
        ...
        History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer("The player stood still")).save()
        successful = False
...
def act_talk(player:Character, target:Monster, action:list):
    if target is None:
        ...
        History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer(response)).save()
        return False
        ...
        try:
            ...
            if target_id.isdigit():
                ...
                if target is None:
                    ...
                    if len(possible_targets) == 0:
                        History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer(f"There's no monster called {target_id} in your range.")).save()
                    else:
                        target = possible_targets[0]
                        History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer(f"There's no monster called {target_id} in your range, talking to {target.name} instead.")).save()
        except:
            pass
    ...
    History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer(response))
...
def act_levelup(player:Character, action:list):
    if player.exp >= player.exp_top:
        ...
        History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer(f"You don't have enough EXP to level up.")).save()
        return False
...
def act_use(player:Character, action:list):
    ...
    if len(action) == 1:
        History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer(f'You need to specify the item you want to use.<br>If you have problems using items, then replace the spaces with "_" i.<br>For example: health_potion.')).save()
    ...
    if item_to_use in ['health potion', 'potion', 'hp potion', 'hp']:
        ...
        if successful:
            ...
            player.save()
            History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer(f'{player.name} used a health potion. 50 HP restored!')).save()
            return True
        ...
        if successful:
            try:
                ...
                player.save()
                History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer(f'{player.name} used a go back bone.')).save()
                return True
            except:
    ...
    if not successful:
        History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer(f"You have no {item_to_use}s.")).save()
        return False
...
def act_equip(player:Character, action:list):
    ...
    if len(possible_treasures) == 0:
        ...
        History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer(f"There's no {treasure_to_take} to take here.")).save()
        successful = False
    else:
        loot = possible_treasures[0].weapon
        History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer(f'{player.name} equipped {loot.name}.')).save()
        player.weapon = loot
        player.save()
    ...
    if len(possible_treasures) == 0:
        ...
        for treasure in possible_treasures:
            if treasure.treasure_type != 'Weapon':
                ...
                History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer(f'{player.name} got {loot} from {treasure.treasure_type}.')).save()
                player.add_all_to_inventory(loot)
                treasure.delete()
        successful = True
    else:
            ...
            if target_id.isdigit():
                ...
                if target is None:
                    ...
                    if len(possible_targets) == 0:
                        ...
                        target = possible_targets[0]
                        History.objects.create(campaign=player.campaign, author='SYSTEM', text=generate_literature_answer(f"There's no monster called {target_id} in your range, attacking {target.name} instead.")).save()
        elif not target:
