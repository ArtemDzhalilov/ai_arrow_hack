...
@csrf_exempt
def campaignSelection_json(request):
    '''POST LABELS:
    - create_campaign (optional) (default = None)
    ...
    if request.method == "POST":
        if 'create_campaign' in request.POST:
            try:
                ...
                History.objects.create(campaign_id=campaign.id, author='NARRATOR', color='purple',
                                       text=initial_story).save()
            ...
            response_data = {
                "success": False,
                "message": "create_campaign label not found in POST data"
            }
            return JsonResponse(response_data, status=400)
...
@csrf_exempt
def playerSelection_json(request):
    '''POST LABELS:
    - campaign_id   (optional) (default = First not completed)
    - create_player (optional) (default = None)
    ...
    if request.method == "POST":
        ...
        if 'create_player' in request.POST:
            try:
                ...
                character_race = request.POST.get('race')
                character_class = request.POST.get('class') or character_race
                ...
                History.objects.create(campaign_id=campaign_id, author='SYSTEM',
                                       text=f"{new_player.name} has joined the party!").save()
            ...
            response_data = {
                "success": False,
                "message": "create_player label not found in POST data"
            }
            return JsonResponse(response_data, status=400)
        ...
        player_list = [{"id": player.id, "name": player.name, "level": player.level} for player in players]
...
def playerCreation(request):
    ...
    print(races, classes, [i.id for i in weapons])
...
def game(request):
    ...
    if request.method == "POST":
        ...
        print(Campaign.objects.filter(is_completed=False))
...
def get_room_history(request):
    """
    Возвращает историю сообщений для указанного room_id в формате JSON.
    Room ID принимается из тела запроса в формате JSON.
    """
    try:
        # Парсим JSON из тела запроса
        print()
        print(1)
        room_id = request.POST['campaign_id']

        if room_id is None:
            return JsonResponse({'error': 'room_id is required'}, status=400)

        # Получаем историю для указанной комнаты (кампании)
        history_records = History.objects.filter(campaign_id=room_id)

        # Формируем строку истории
        history_string = ''
        for record in history_records:
            history_string += f"{record.author}: {record.text}\n"

        # Возвращаем историю в формате JSON
        return JsonResponse({'history': history_string.strip()}, json_dumps_params={'ensure_ascii': False})
